tinymce.init({
    ...
    valid_elements: 'p,br,b,strong,i,em,h1,h2,h3,h4,h5,h6,ul,ol,li,a[href|target],img[src|alt|width|height],table,tr,td,th,thead,tbody', // Sadece gerekli elementlere izin ver
    valid_children: '+body[p|h1|h2|h3|h4|h5|h6|ul|ol|li|table]', // Body içinde geçerli elementler
    
    // Temizleme kuralları
    cleanup: true,
    cleanup_on_startup: true,
    convert_fonts_to_spans: false,    // Font etiketlerini span'a çevirme
    ...
    // Paste ayarları
    paste_as_text: true,             // Yapıştırılan içeriği düz metin olarak yapıştır
    ...
    setup: function(editor) {
        editor.on('BeforeSetContent', function(e) {
            ...
            if (e.content) {
                ...
                // Gereksiz span'ları kaldır
                e.content = e.content.replace(/<span[^>]*>/gi, '');
                e.content = e.content.replace(/<\/span>/gi, '');
                
                // Gereksiz class ve style özelliklerini kaldır
                e.content = e.content.replace(/\s+class="[^"]*"/gi, '');
                e.content = e.content.replace(/\s+style="[^"]*"/gi, '');
            }
        });
    }
}); 